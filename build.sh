...
podman run --rm -v "$(pwd)":/app:Z build_frontend /bin/bash -c "npm install"
...
#podman run --rm -p 9087:3000 -v "$(pwd)":/app:Z \
#                -v "$(pwd)"/unrpapp:/app/unrpapp:Z \
#                build_frontend /bin/bash -c "cd /app/unrpapp && npm start"